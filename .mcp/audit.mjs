...
try {
	...
	if (target) {
		arguments_.target =
			target.startsWith("file://") || target.startsWith("http")
				? target
	...
	const diagnosticsText =
		diagnostics && Object.keys(diagnostics).length
			...
			: "";
	const ocr = result.structuredContent?.ocr;
	const ocrText =
		ocr?.text && String(ocr.text).trim()
			? `OCR:\n${String(ocr.text).trim()}`
			: "";

	console.log(
		[text, diagnosticsText, ocrText].filter(Boolean).join("\n\n") ||
